...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build an S&P500 Qlib dataset from Yahoo Finance.")
    parser.add_argument("--source-qlib", default="~/.qlib/qlib_data/us_data")
    parser.add_argument("--target-dir", default="data/qlib/us_data_nextday2025")
    ...
    for batch in chunks(download_symbols, args.batch_size):
        print(f"Downloading {len(batch)} symbols: {batch[0]} ... {batch[-1]}")
        data = yf.download(
            [to_yahoo_symbol(symbol) for symbol in batch],
            start=args.start,
            end=args.end,
            auto_adjust=True,
            group_by="ticker",
        ...
        frames.update(extract_batch(data, batch))
        time.sleep(0.5)
...
def read_instrument_intervals(path: Path) -> dict[str, list[tuple[str, str]]]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if len(parts) < 3:
            continue
        intervals.setdefault(parts[0], []).append((parts[1], parts[2]))
    return intervals
...
def extract_batch(data: pd.DataFrame, symbols: list[str]) -> dict[str, pd.DataFrame]:
    out = {}
